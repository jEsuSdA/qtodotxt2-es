...
class TaskHtmlizer(object):
    ...
    def task2html(self, task):
        ...
        if task.completion_date:
            html += ' <font color="{1!s}">(completada: {0!s})</font>'.format(task.completion_date, self.complColor)
        if task.creation_date:
            html += ' <font color="{1!s}">(creada: {0!s})</font>'.format(task.creation_date, self.complColor)
        return html
    ...
    def _htmlizeDueDate(self, task, string):
        if not task.due:
            return ('<b><font style="color:{1!s}">*** {0!s} Fecha errónea,'
                    ' se esperaba yyyy-mm-dd o yyyy-mm-ddThh:mm. ***</font></b>'.format(string, self.errorColor))
