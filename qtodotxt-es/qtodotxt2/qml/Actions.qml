...
Item {
    ...
    QtObject {
        ...
        property bool showFuture: true
        property bool showHidden: false
        property string sortingMode: "default"

        function save(_url) { /* noop */ }
        function open(_url) { /* noop */ }
        function completeTasks(_idxs) { /* noop */ }
        function archiveCompletedTasks() { /* noop */ }
    }
    ...
    property Action newTask: Action {
        ...
        enabled: !taskListView.editing
        onTriggered: taskListView.newTask('')
    }
    ...
    property Action newTaskFrom: Action {
        ...
        onTriggered: taskListView.newFromTask()
    }
    ...
    property Action completeTasks: Action {
        ...
        onTriggered: {
            ...
            mc.completeTasks(taskListView.getSelectedIndexes())
    ...
    FileDialog {
        ...
        // ✅ Nunca undefined: QUrl válido siempre
        folder: (mc.docPath && mc.docPath.length > 0) ? mc.docPath : Qt.resolvedUrl("file:///")

        onAccepted: {
            if (fileDialog.selectExisting) mc.open(fileUrl)
            else mc.save(fileUrl)
        }
    }
