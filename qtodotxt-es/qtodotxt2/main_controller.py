...
#        logger.debug('MainController, saving file: %s.', path)
    ...
    # Añadir slot que abra ventana Kanban
    @QtCore.pyqtSlot()
    def openKanbanView(self):
        from qtodotxt2.kanban_controller import KanbanController
        from qtodotxt2.kanban_window import KanbanWindow

        if not hasattr(self, "kanban_controller") or self.kanban_controller is None:
            self.kanban_controller = KanbanController(self)
    ...
    def updateTaskPriority(self, line_index, new_priority):
        # Buscar tarea por índice
        task = self._file.tasks[line_index]
        # Actualizar prioridad usando property setter
    ...
    def docPath(self):
        return QtCore.QUrl.fromLocalFile(
            QtCore.QStandardPaths.writableLocation(QtCore.QStandardPaths.DocumentsLocation)
        )


    currentDirChanged = QtCore.pyqtSignal()

    @QtCore.pyqtProperty('QUrl', notify=currentDirChanged)
    def currentDir(self):
        if getattr(self._file, "filename", None):
            import os
            return QtCore.QUrl.fromLocalFile(os.path.dirname(self._file.filename))
        return self.docPath
