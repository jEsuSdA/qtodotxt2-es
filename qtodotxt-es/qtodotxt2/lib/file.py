...
# MODIFICADA getAllDueRanges para mostrar TODOS los filtros, aunque no haya una tarea que los cumpla
# En la versión original sólo se muestra un filtro si hay, al menos, una tarea que lo cumple
...
# Dentro de la clase File en file.py

    def getAllDueRanges(self):
        # 1. Define la lista de todos los filtros de vencimiento que SIEMPRE quieres mostrar.
        all_due_filters = [
            DueTodayFilter(),
            DueTomorrowFilter(),
            DueThisWeekFilter(),
        ...
        # 2. Inicializa el diccionario dueRanges AHORA, con todos los filtros y contadores a cero.
        #    La clave es el objeto filtro, el valor es la lista [incompletas, completas]
        dueRanges = {flt: [0, 0] for flt in all_due_filters}

        # 3. Itera sobre las tareas (como antes)
        for task in self.tasks:
            idx = 1 if task.is_complete else 0 # Índice para incompletas (0) o completas (1)

            # 4. Comprueba la tarea contra cada uno de los filtros predefinidos
            for flt in all_due_filters: # Es importante usar la misma lista predefinida
                if flt.isMatch(task):
                    # 5. Si hay coincidencia, simplemente incrementa el contador correspondiente.
                    #    Ya no necesitas comprobar si 'flt' está en dueRanges, porque AHORA SIEMPRE está.
                    dueRanges[flt][idx] += 1
                    # Opcional: Si sabes que una tarea solo puede coincidir con UN tipo de
                    # vencimiento (ej: no puede ser Hoy Y Mañana a la vez), podrías
                    # añadir un 'break' aquí para optimizar un poco, pero no es esencial.
                    # break

        # 6. Devuelve el diccionario. Ahora contendrá TODOS los filtros,
        #    algunos con contadores > 0 y otros con [0, 0].
        return dueRanges
